...
class ThemeViewController: UIViewController, MGLMapViewDelegate, CLLocationManagerDelegate {
    ...
    // Use a MBXTheme from Themes.swift, or create a Color object that contains the necessary colors and use it to set the theme.
          var viewControllerTheme : Theme? = MBXTheme.greenTheme
    
    //Edmonton
    //Latitude‎: ‎53.631611
    //Longitude‎: ‎-113.323975
    var centerCoordinate = CLLocationCoordinate2D(latitude: 53.631611, longitude: -113.323975) // This will serve as the center coordinate if the user denies location permissions.
    ...
    let uniqueIdentifier = "location_name" // Replace this with the property key for a value that is unique within your data. Do not use coordinates.
    ...
    func drawPointData(data: Data) {
        ...
        let source = MGLShapeSource(identifier: "wifi-locations", shape: feature, options: nil)
        style.addSource(source)
        
        style.setImage((viewControllerTheme?.defaultMarker)!, forName: "wifi-256") // Set the default item image.
        style.setImage((viewControllerTheme?.selectedMarker)!, forName: "wifi-256") // Set the image for the selected item.
        
        let symbolLayer = MGLSymbolStyleLayer(identifier: "wifi-locations", source: source)
        
        symbolLayer.iconImageName = MGLStyleValue(rawValue: "wifi-256")
    ...
    func changeItemColor(feature: MGLFeature) {
        let layer = mapView.style?.layer(withIdentifier: "wifi-locations") as! MGLSymbolStyleLayer
        if let name = feature.attribute(forKey: "location_name") as? String {
            ...
            layer.iconImageName = MGLStyleValue(interpolationMode: .categorical,
                                                sourceStops: [name: MGLStyleValue<NSString>(rawValue: "wifi-256")],
                                                attributeName: "location_name",
                                                options: [.defaultValue: MGLStyleValue<NSString>(rawValue: "wifi-256")])
        } else {
...
class CustomItemView : UIView {
    ...
    public func updateLabels() {
        if let name : String = selectedFeature.attribute(forKey: "location_name") as? String {
            containerView.itemNameLabel.text = name
        }
        if let hours : String = selectedFeature.attribute(forKey: "wifiprovider") as? String {
            containerView.itemHourLabel.text = hours
        }
        if let description : String = selectedFeature.attribute(forKey: "address") as? String  {
            containerView.itemDescriptionLabel.text = description
        }
        
        if let number : String = selectedFeature.attribute(forKey: "status") as? String  {
